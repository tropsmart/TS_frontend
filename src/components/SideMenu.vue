...
<!--
    <v-navigation-drawer v-model="drawer" fixed app clipped class="drawer-style" id="style-1">
        <v-list dense class="pt-3 white--text">
            <v-list-item v-for="source in sources" :key="source.id" @click="selectSource(source.id)">
                <v-list-item-action>
                    <v-avatar size="32px">
                        <img clas="img-circle elevation-7 mb-1" :src="urlToLogo(source)" : alt="source.name"/>
                    </v-avatar>
                </v-list-item-action>
                <v-list-item-action>
    ...
    <v-container>
        <v-layout>
            <v-flex>
                <v-img src="https://carlos18mz.github.io/TropSmartPatrones.github.io/img/bg-masthead.jpg"></v-img>
            </v-flex>
        </v-layout>
    </v-container>
</template>
...
<style scoped>
    ...
    #style-2::-webkit-scrollbar-thumb{
        ...
        background-image: -webkit-linear-gradient(90deg, rga(255,255,255,.2) 25%,
            transparent 25%,
            transparent 50%,
    ...
    #style-3::-webkit-scrollbar-track {
        -webkit-box-shadow: inser 0 0 6px rgba(0,0,0,.3);
        background-color: #F5F5F5;
    }
